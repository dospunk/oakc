...
impl Target for Lua {
    ...
    fn std(&self) -> String {
        String::from(include_str!("std/std.lua"))
    }

    fn core_prelude(&self) -> String {
        String::from(include_str!("core/core.lua"))
    }
    ...
    fn begin_while(&self) -> String {
        String::from("while machine_pop(vm) ~= 0 do\n")
    }
    ...
    fn compile(&self, code: String) -> Result<()> {
        if let Ok(_) = write("main.lua", code) {
            return Result::Ok(());
        }
